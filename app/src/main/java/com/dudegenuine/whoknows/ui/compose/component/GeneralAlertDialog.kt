...
fun GeneralAlertDialog(viewModel: IActivityViewModel) {
    val state = viewModel.dialogContent.collectAsState(initial = null)

    if(state.value is DialogState) with(state.value as DialogState){
        val buttonText = button
            .replaceFirstChar { it.uppercase() }
        fun onDismissedPressed()
            { onDismissed?.invoke() ?: viewModel.onShowDialog(null) }
        fun onButtonPressed() {
            onSubmitted?.invoke()
            onDismissedPressed()
        }
